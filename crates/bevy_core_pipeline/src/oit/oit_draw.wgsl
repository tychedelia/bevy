...
#import bevy_pbr::mesh_view_bindings::{push_constants, view, oit_layers, oit_layer_ids, oit_settings}
...
#ifdef MULTIPLE_LIGHT_PROBES_IN_ARRAY
    if color.a < oit_settings[view_index].alpha_threshold {
#else
...
#ifdef MULTIPLE_LIGHT_PROBES_IN_ARRAY
    if layer_id >= oit_settings[view_index].layers_count {
        // force to store the oit_layers_count to make sure we don't
        // accidentally increase the index above the maximum value
        atomicStore(&oit_layer_ids[screen_index], oit_settings[view_index].layers_count);
#else
