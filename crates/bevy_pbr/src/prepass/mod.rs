...
use crate::{
    ...
    PrepassVertexShader, RenderLightmaps, RenderMaterialInstances, RenderMeshInstanceBatches,
...
pub fn queue_prepass_material_meshes(
    ...
    for (extracted_view, visible_entities) in &views {
        ...
        // respecialization (atomic `Mesh3d` + GPU-authored
        // `GpuBatchedMesh3d`).
                ...
                RenderPhaseType::AlphaMask => {
                    if deferred {
                        if let Some(alpha_mask_deferred_phase) =
                            alpha_mask_deferred_phase.as_mut()
                        {
