...
//
// Each invocation simulates one particle and writes its transform into the
// mesh preprocessing input buffer, at the slot the `GpuInstances3d`
// reservation assigned to it. Retired particles are hidden by writing a
// `alive` of zero into the culling data buffer.

#import bevy_pbr::mesh_preprocess_types::{MeshInput, MeshCullingData}
...
fn simulate(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Orient each particle to face the attractor.
    let to_mouse = params.mouse_world_pos.xyz - state.pos.xyz;
    ...
    mesh_culling_buffer[slot].alive = select(1.0, 0.0, dead);
}
